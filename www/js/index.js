define(["require", "exports", "./Vec3.js"], function (require, exports, Vec3) {
    "use strict";
    ...
    const lightDirection = (new Vec3.Vec3(.15, -1, .25)).normalize();
    function rotatePitchRoll(vec, pitch, roll) {
        const cp = Math.cos(pitch);
        const sp = Math.sin(pitch);
        const cr = Math.cos(roll);
        const sr = Math.sin(roll);
        return new Vec3.Vec3(vec.x * cp + vec.z * sp, vec.x * sp * sr + vec.y * cr - vec.z * sr * cp, -vec.x * sp * cr + vec.y * sr + vec.z * cp * cr);
    }
    const shadowXFactor = lightDirection.dot(new Vec3.Vec3(-1, 0, 0));
    const shadowYFactor = lightDirection.dot(new Vec3.Vec3(0, -1, 0));
    ...
    const unLightDirection = new Vec3.Vec3(-lightDirection.x, -lightDirection.y, lightDirection.z);
    const backward = new Vec3.Vec3(0, 0, 1);
